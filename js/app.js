...
class UI {
    ...
    /**
     * Flips (by default vertically) selected cells.
     * @param {KeyboardEvent} ev - If CTRL key is pressed then cells
     *  are flipped horizontally.
     */
    static flip(ev) {
        let axis = ev.ctrlKey;
        this.transform((point, pivot) => {
            point.x = Math.floor((axis ? -1 : 1) * point.x + (axis ? 2 : 0) * pivot.x);
            point.y = Math.floor((axis ? 1 : -1) * point.y + (axis ? 0 : 2) * pivot.y);
            return point;
        });
    }

    /**
     * Transforms points in the clipboard using function provided in the call.
     * @param {function} fx - The function which transforms coordinates of the point.
     * @param {(object|Point)=} pivot - The center point of the transformation.
     *  If the pivot isn't specified then the center point will be choosen.
     */
    static transform(fx, pivot) {
        if (this.selectState != 'selected')
            return;
        ...
        if (!pivot) {
            let bb = Life.getBoundingBox(this.clipboard);
            pivot = {
                x: bb.left + (bb.width  - 1) / 2,
                y: bb.top  - (bb.height - 1) / 2
            };
        }
        let points = this.clipboard.map(p => fx($.deepCopy(p), pivot));
        this.cut();
        this.life.load(points);
